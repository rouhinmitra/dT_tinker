...
#Call EE
import ee
def fexp_et(image, Rn24hobs,name=None,number=None):
    ...
    if name=="LE":
        le= i_lambda_ET.expression(str(number)+'*LE', {'LE': i_lambda_ET.select('LE')}).rename("LE");
        # ls_trial=ls_trial.addBands(rs_down,overwrite=True).toFloat()
        i_lambda_ET=i_lambda_ET.addBands(le,overwrite=True)
